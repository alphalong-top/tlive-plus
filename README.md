...
This setup uses Codex App's internal `CODEX_APP_SERVER_WS_URL` transport. A
small launchd-managed loopback bridge forwards `ws://127.0.0.1:43873` to the
managed Unix socket. The loopback URL matters: Codex App routes non-local
WebSocket hostnames through its built-in SOCKS fallback. tlive never patches the
Codex App or binary. `tlive stop` disconnects tlive but leaves the bridge and
managed app-server alive, so Codex App keeps working. `tlive codex shared off`
removes the bridge and App environment setting; fully restart Codex App to apply
that rollback.
