...
这套配置使用 Codex App 内部的 `CODEX_APP_SERVER_WS_URL`,并由 launchd 常驻的
轻量 bridge 将 `ws://127.0.0.1:43873` 转发到 managed Unix socket。必须使用
loopback URL,因为 Codex App 会把非本地主机的 WebSocket 连接送入内置 SOCKS
fallback。它不会修改 Codex App 或二进制。`tlive stop` 只断开 tlive,bridge 和
managed app-server 继续运行,所以不会带停 Codex App。`tlive codex shared off`
会移除 bridge 和 App 环境设置,回滚后同样需要重启 App。
...
```jsonc
{
  ...
  "codex": {
    ...
    "autoRetry": {
      "enabled": true,                 // 默认 true
      "delaysSec": [60, 180, 300]      // 各次重试等待秒数；数组长度即重试次数
    }
  },
  "approvals": {
